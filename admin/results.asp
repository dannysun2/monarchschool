...
convertDate = mid(convertDate,7,4) + "-" + mid(convertDate,1,2) + "-" + mid(convertDate,4,2)
end function

sub pass2

from_date = request.form("from_dt")
if mid(from_date,2,1) = "/" then from_date = "0" + from_date
if mid(from_date,5,1) = "/" then from_date = mid(from_date,1,3) + "0" + mid(from_date,4,6)
...
<div class="block3">
        <div class="block_head">
          ...
          <ul>
             <li>DATE RANGE : <%=cstr(from_date)%> to <%=cstr(to_date)%> </li>
             <li>LEVEL : <%=cstr(level)%></a></li>
          </ul>
        </div>
