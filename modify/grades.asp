...
set rs=nothing
  ...
  set rs=Server.CreateObject("ADODB.Recordset")
  SQLString2="SELECT * FROM grades WHERE student_id=" +cstr(sid) +" AND grades_id=" +cstr(gid)
...
error_count = error_count + 1
	...
	emotion_error = "Emotion Control grade may not be over 100."
	error_count = error_count + 1
	elseif cint(emotion_val) < 0 then
	emotion_error = "Emotion Control grade may not be under 0."
	error_count = error_count + 1
	end if
end if
